...
<Callout type="pitfall" title="The subclass explosion this avoids">
...
<li>Each decorator has a single, focused responsibility (ties to <a href={withBase('/solid/srp/')}>SRP</a>)</li>
</ul>
</div>
<div>
...
- Explicitly contrast with Proxy (see <a href={withBase('/cheatsheet/')}>Cheat Sheet</a>): Decorator *adds* new
...
- <a href={withBase('/structural/proxy/')}>Proxy</a> — structurally almost identical, but with a different intent: controlling access rather than adding behavior.
- <a href={withBase('/structural/adapter/')}>Adapter</a> — changes the interface; Decorator deliberately keeps it the same.
- <a href={withBase('/structural/composite/')}>Composite</a> — both rely on recursive, uniform interfaces, but Composite models part-whole trees, not layered behavior.
