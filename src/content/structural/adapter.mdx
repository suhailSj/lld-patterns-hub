...
<Callout type="warn" title="Why not just edit the legacy class?">
...
<a href={withBase('/solid/ocp/')}>Open/Closed</a>. Adapter lets both sides stay untouched.
</Callout>
...
- <a href={withBase('/structural/facade/')}>Facade</a> — both wrap something else, but Facade simplifies a whole subsystem's interface for convenience; Adapter makes one existing interface match another that a client already expects.
- <a href={withBase('/structural/decorator/')}>Decorator</a> — same interface, added behavior — the mirror image of Adapter's different interface, same behavior.
- <a href={withBase('/structural/bridge/')}>Bridge</a> — Bridge is designed up front to let abstraction and implementation vary independently; Adapter is typically retrofitted after the fact to reconcile two things that weren't designed together.
