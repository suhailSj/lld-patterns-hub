...
- If you only ever create one kind of object (not a *family* of related ones), a plain <a href={withBase('/creational/factory-method/')}>Factory Method</a> is simpler and sufficient.
...
consistent with each other. See the <a href={withBase('/cheatsheet/')}>Cheat Sheet</a> for the full comparison table.
</Callout>

## Related patterns

- <a href={withBase('/creational/factory-method/')}>Factory Method</a> — Abstract Factory is commonly implemented as several Factory Methods grouped behind one interface.
- <a href={withBase('/creational/builder/')}>Builder</a> — use Builder instead when a single product is complex to assemble step-by-step; use Abstract Factory when the complexity is in choosing a *consistent family*, not assembling one object.
- <a href={withBase('/structural/bridge/')}>Bridge</a> — both patterns decouple an abstraction from multiple implementations, but Bridge is about letting one abstraction vary independently from its implementation, not about creating families of objects.
