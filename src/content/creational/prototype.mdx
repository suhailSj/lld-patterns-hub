...
<Callout type="pitfall" title="Shallow copy surprises">
...
- <a href={withBase('/creational/builder/')}>Builder</a> — an alternative when the "expensive" part is *assembly logic* rather than the raw cost of allocation; Prototype instead reuses an already-assembled instance.
- <a href={withBase('/creational/singleton/')}>Singleton</a> — the opposite instinct: Singleton insists on exactly one instance, Prototype exists specifically to make new instances cheap.
- <a href={withBase('/fundamentals/immutability/')}>Immutability</a> — an immutable prototype removes the shallow-copy risk entirely, since there's nothing mutable to accidentally share.
