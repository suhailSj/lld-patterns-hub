...
<Callout type="warn" title="Watch for silent failures and ordering assumptions">
...
- <a href={withBase('/behavioral/command/')}>Command</a> — observers are sometimes implemented as queued Commands when notification needs to be asynchronous or retryable.
- <a href={withBase('/behavioral/chain-of-responsibility/')}>Chain of Responsibility</a> — contrast: Observer notifies every registered listener; Chain of Responsibility stops at the first handler that handles the request.
- <a href={withBase('/solid/dip/')}>Dependency Inversion Principle</a> — the subject depends only on the small `OrderObserver` abstraction, never on concrete observer classes.
