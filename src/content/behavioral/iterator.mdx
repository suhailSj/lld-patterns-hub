...
- <a href={withBase('/structural/composite/')}>Composite</a> — Iterator is commonly used to traverse a Composite tree without exposing its recursive structure to callers.
- <a href={withBase('/creational/factory-method/')}>Factory Method</a> — a collection class often exposes a factory method (`iterator()`, `__iter__`) that returns the appropriate iterator for its internal structure.
