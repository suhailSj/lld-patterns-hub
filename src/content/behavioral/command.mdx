...
- <a href={withBase('/behavioral/observer/')}>Observer</a> — commands are sometimes dispatched in response to an observed event.
- <a href={withBase('/behavioral/strategy/')}>Strategy</a> — both wrap behavior in an object, but Strategy is chosen once per operation to vary an algorithm; Command represents a single discrete action, often stored for later/undo.
- <a href={withBase('/behavioral/chain-of-responsibility/')}>Chain of Responsibility</a> — can be combined with Command to route a command to whichever handler is able to process it.
