...
<h4>✅ Pros</h4>
<ul>
<li>New algorithms/strategies are added without touching existing ones (ties to <a href={withBase('/solid/ocp/')}>OCP</a>)</li>
...
Strategy is nearly identical in shape to <a href={withBase('/behavioral/state/')}>State</a> — see the
<a href={withBase('/cheatsheet/')}>Cheat Sheet</a> for the exact distinction (who chooses the implementation, and
whether the implementations know about each other).
</Callout>

## Related patterns

- <a href={withBase('/behavioral/state/')}>State</a> — nearly identical structure, different intent: State's implementations transition between each other; Strategy's are chosen explicitly and independently.
- <a href={withBase('/creational/factory-method/')}>Factory Method</a> — often used to decide which concrete strategy to construct.
- <a href={withBase('/solid/ocp/')}>Open/Closed Principle</a> — Strategy is the primary behavioral pattern for satisfying OCP when the varying part is "which algorithm runs."
