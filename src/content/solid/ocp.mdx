...
<Callout type="pitfall" title="The growing if/elif chain">
...
- <a href={withBase('/behavioral/strategy/')}>Strategy</a> — the behavioral pattern most directly modeling OCP: interchangeable algorithms behind one interface.
- <a href={withBase('/creational/factory-method/')}>Factory Method</a> — often used alongside OCP to decide *which* concrete strategy/discount to instantiate without a switch statement leaking into client code.
- <a href={withBase('/solid/srp/')}>Single Responsibility Principle</a> — SRP is usually the prerequisite: you can't cleanly extend a class that already has five unrelated responsibilities.
